  ...
  function decorateWorldBossPanel({ root, heading }) {
    ...
    for (const card of root.querySelectorAll(".compact-panel")) {
      ...
      if (action) {
        ...
        const actionState = /fighting/i.test(actionText) ? "fighting" : /prejoined/i.test(actionText) ? "prejoined" : /\b(?:prejoin|join)\b/i.test(actionText) ? "join" : "idle";
